...
class CPPFileProcessingView(APIView):
    def post(self, request, format=None):
        ...
        if compile_process.returncode != 0:
            return Response({"message": compile_error.decode("utf-8"), "status": status.HTTP_400_BAD_REQUEST})
        ...
        if execute_process.returncode != 0:
            return Response({"message": execute_error.decode("utf-8"), "status": status.HTTP_400_BAD_REQUEST})
